...
void endProg(void)
{
   ...
   if(QFileInfo("/mnt/ramdisk/state").exists()) QFile::remove("/mnt/ramdisk/state");
...
// Daemonize if not debug mode
    ...
    if(debug==1){
      freopen( "/home/work/ti_log", "w", stdout);
      freopen( "/home/work/ti_err", "w", stderr);
    }
  ...
  // start program
  timer=new TTimerBehave();
  if(timer) timer->start(QThread::NormalPriority);
  //a.exec();
  while(1) {a.processEvents(); sleep(1); if(timer->isFinished()) {delete timer; break;} }
  if(QFileInfo("/mnt/ramdisk/status").exists()) QFile::remove("/mnt/ramdisk/status");
  if(QFileInfo("/mnt/ramdisk/state").exists()) QFile::remove("/mnt/ramdisk/state");
  return 0;
} //end try
