...
TTimer::TTimer(QString devName): TSerial()
{
  device=devName;
  qDebug()<<"Constructor of object TTimer start";
  strErrors<<"None"<<" "<<" "<<" "<<" "<<" "<<" "<<" "<< // 7 Atmega162 errors
             ...
             "Setup address not valid"<<"Setup chanel not valid"<<"Setup data not valid"<< //17
...
//    printf("%d %d %d\n",ch,t,w);
    timerError=writeRegTime(ch,t); if(timerError) return timerError;
    timerError=writeRegWidth(ch,w); if(timerError) return timerError;
  }
  file.close();
  return timerError;
}
