...
static void child_handler(int signum)
{
  ...
  break;
  case SIGTERM:{ //stop after demonize proces
...
//syslog( LOG_ERR, "Program PID %d parent PID %d.",getpid() ,getppid() );
  /* already a daemon */
//  if ( getppid() == 1 ) {
...
//  } // run from init
  ...
  pid = fork();
//  syslog( LOG_ERR, "Program PID %d parent PID %d fork %d.",getpid() ,getppid(),pid );
  if (pid < 0) {
...
// Daemonize if not debug mode
  ...
  if(btnproc) btnproc->start(QThread::NormalPriority);
  btnproc->wait();

  //QCoreApplication::processEvents();
//  device->start(QThread::NormalPriority);
 // return a.exec();
  if(btnproc) delete btnproc;
   exit(EXIT_SUCCESS);
} //end try
catch(...)
  {
