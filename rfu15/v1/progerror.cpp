    ...
    for(uint i=0;i<bitError.size();i++){
      if(bitError.test(i))
        switch(i)
        {
          ...
          case INFILE_ERROR: devErrorsText.append(tr("File with dataform don't exist.")); break;
          case SYNCHR_ERROR: devErrorsText.append(tr("Synchr pulse don't present.")); break;
          case PS_HW_ERROR: devErrorsText.append(tr("HW return status of error. May be HW is absent or fatal error.")); break;
          default:;
        }
    }
  }
