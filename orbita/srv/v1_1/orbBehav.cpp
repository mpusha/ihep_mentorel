...
void TOrbBehav::run()
{
  ...
  emit signalErrStatusMsg("Begin   Work Orb");
...
// processing hardware errors in device
      ...
      case UPDATE_STATUS_STATE: {
        ...
        if(status&0x40) emit signalErrStatusMsg("Warning In Local");
        else if(status&0x10)
          ...
          if(status&0x02) emit signalErrStatusMsg("OK! Beg measure ");
          else {
            if((oldStatus&0x02)==0){
              emit signalErrStatusMsg("OK! End measure ");
...
void TOrbBehav::createOrbData(void)
{
  int wt,nwt;
  double tC;
  if(modeRegime&2) {wt=0; nwt=1;} else { wt=1; nwt=0;} // work/test
...
void TOrbBehav::readMeasure(double realT[timePoints],char to[timePoints], double orbitaR[][timePoints], double orbitaZ[][timePoints])
{
  for(int i=0;i<timePoints;i++){
