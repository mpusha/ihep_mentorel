...
TOrbBehav::~TOrbBehav()
{
  qDebug()<<"Start TOrbBehav destructor.";
  if(!abort) {
    abort=true;
    wait(2000);
  }
  qDebug()<<"Stop TOrbBehav destructor.";
}
...
void TOrbBehav::initialDevice(void)
{
  ...
  usr_configureFPGA((char*)QString(workDirPath+"/orbita_project.rbf").toStdString().c_str());
  usr_rst(); //reset FPGA
}
...
int TOrbBehav::readCA(void)
{
  int axis;
  bool ok;
  for(int i=0;i<allAxis;i++) {
    absAxixR[i]=false; absAxisZ[i]=false;
