  ...
  if(db) {
    if(!db->getDBStatus()){
      ...
      if(orb) orb->start(QThread::NormalPriority);
        ...
        delete db;
        printf("Can't create TOrbBehav object. Program was terminated.\n");
        syslog( LOG_ERR,"Can't create TOrbBehav object. Program was terminated.");
        exit(EXIT_FAILURE);
      }
      db->start(QThread::NormalPriority);
      // normal start  program with DB
    }
    else {
...
//  device->start(QThread::NormalPriority);
  while(!orb->isRunning());
  if(argc>2){
    if(!strcmp(argv[2],"tm")){orb->setCalibration(1);}
    else
      if(!strcmp(argv[2],"tv")){orb->setCalibration(2);}
    else
      if(!strcmp(argv[2],"tg")){orb->setCalibration(3);}
  }
  //while(1) {a.processEvents(); if(db->isFinished()) {delete db; break;} }
  return a.exec();
} //end try
catch(...)
  {
