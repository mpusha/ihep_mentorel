    ...
    if(timerEv){
      ...
      if(writeHWReq){
        if(update){
          ...
          printf("All parameters  was writen successfully!\n");
        }
        writeHWReq=0;
      }
...
// end write data into current DB
        ...
        viewWWW=1;
        qDebug()<<"DB - Update DB end";
      }
      if(viewWWW){
        qDebug()<<"WWW - Update date start";
        if((cur_par.graph==0)||(cur_par.graph==3)){ // prepere for view of current orbit
...
// processing status or error request
    if(haveErrStatusMsg){
      ...
      qDebug()<<"DB - Received error or status message:"<<msg_cur;
...
// create data for current measure
...
// Math calculation and write data into DB
...
// prepare status and data for WWW app
...
// write orb data to file for use in WWW app
