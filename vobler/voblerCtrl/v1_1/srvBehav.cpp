...
// Sample device request from timer
      case GET_REQ_AND_STATUS_STATE: { // get requests from client and status from HW
        ...
        if(stErr) {
          ...
          phase=DEVICE_ERROR_STATE;
          if(cntErrReq++>2) allStates[INITIAL_STATE]=INITIAL_STATE; // initial system again
        }
        else{
          cntErrReq=0;
...
// load FPGA
 // devError.setDevError(TDevErrors::FPGA_LOAD_ERROR);
  if(dev->usr_initalBBHW())  { devError.setDevError(TDevErrors::GPIO_ERROR); return; } // initial pins and spi
...
// get power On/Off get can read/write status and signature
  ...
  if(st&0x20) pulsePS=true;
  if((pulsePS==true)&&((st&0x20)==0)) { measReq=true; pulsePS=false;}
  qDebug()<<"st"<< (void *)st;
}
/*
...
void TDtBehav::wrDataReadData(bool withWr)
{
  ...
  if(withWr){
    ...
    dev->usr_waitCanRead();
    dev->usr_WrDACFile(workDirName+"data/data/"+selFile);
    ...
    dev->usr_waitCanRead();
    dev->usr_RdADCFile(workDirName+"data/ctrl/meas.dat");
