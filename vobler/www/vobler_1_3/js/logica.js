...
function fileForPSSelect(id){
  ...
  webix.ajax().post("data/csvtojson.php",{filename:"./data/"+selfile.title,delfile:0}).then( function (result) {
    $$("graphPsDataSet").clearAll();
    var meas=result.json();
    var mi=meas[meas.length-1].min;
    var ma=meas[meas.length-1].max;
    var st=meas[meas.length-1].step;
    meas.pop(); // remove last data min max step
    $$("graphPsDataSet").parse(meas); 
    $$("graphPsDataSet").define({ yAxis:{start:mi,end:ma,step:st,title:"I,A"}});
    $$("graphPsDataSet").define( {series:[ {value:"#y1#",line:{color:"#1293f8",width:2}},{value:"#y2#",line:{color:"#66cc00",width:2}} ] } );
    $$("graphPsDataSet").refresh();
    $$("selFile").setValue(selfile.title);
    $$("selFile").refresh();
    $$("tabview").getTabbar().setValue("t2");
    $$("buttonWrite").enable();
    $$("buttonLoad").disable();
...
function uploadCompleate(item, response){
  webix.ajax().post("./data/dataupload.php").then( function (result){
  $$("fileList").clearAll();
...
function writePressed(id,e){
  ...
  webix.ajax().post("./data/dataupload.php",{filesel:selfile.title}).fail(function writePressed(result){
...
function setStatus(obj){
  ...
  if(obj[0].statusReq==0) obj[0].statusReq=localizator.lex_statusStart;
  if(obj[0].statusReq==1) obj[0].statusReq=localizator.lex_statusProcessing;
  if(obj[0].statusReq==2) obj[0].statusReq=localizator.lex_statusDataWriten;
  if(obj[0].statusReq==3) obj[0].statusReq=localizator.lex_statusUNDEF;
  if(obj[0].statusReq==4) obj[0].statusReq=localizator.lex_statusFNF;
  $$("statusWrite").refresh();
}
