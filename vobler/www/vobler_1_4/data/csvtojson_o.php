...
if(file_exists($fname)){
  if (($handle = fopen($fname, "r")) !== FALSE) {
    ...
    while (($data = fgetcsv($handle)) !== FALSE) {
      $i = 0;
      if(abs($data[$i+3])>$mx) $mx=abs($data[$i+3]); if(abs($data[$i+4])>$mx) $mx=abs($data[$i+4]); // define max 
      if(!($l%5)) 
      $column[]=array($column_headers[0]=>(float)$data[$i]*1000,$column_headers[1]=>(float)$data[$i+3],$column_headers[2]=>(float)$data[$i+4]);
  ...
  $column[]=array("t"=>(float)0,"y1e"=>(float)0,"y2e"=>(float)0);
